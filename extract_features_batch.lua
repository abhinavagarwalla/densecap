...
cmd:option('-image_height', 360)
cmd:option('-image_width', 360)
...
local function run_image(model, img_paths, opt, dtype)
  -- Load, resize, and preprocess image
  print(#img_paths)
  -- print(img_paths)
  images_all = torch.DoubleTensor(#img_paths, 3, opt.image_width, opt.image_height)
  for i, image_path in ipairs(img_paths) do
    local img = image.load(image_path, 3)
    img = image.scale(img, opt.image_width, opt.image_height):float()
    ...
    images_all[i]:copy(img_caffe)
  end

  local boxes_scores, feats = model:extractFeaturesBatch(images_all:type(dtype))
  print(boxes_scores)
...
local function main()
  ...
  local boxes, feats = run_image(model, image_paths, opt, dtype)
